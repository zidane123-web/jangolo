...
/// Données passées depuis la liste (fictives pour l’instant).
class ArticleDetailData {
  final String name;
  final String sku;
  final String categoryLabel; // ex: "Téléphones"
  final double buyPrice;      // prix d'achat (HT)
  final double sellPrice;     // prix de vente (TTC/HT selon ton choix)
  final int qty;              // stock disponible
...
/// Ecran de fiche article (avec données fictives complétées localement).
class ArticleDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final sales30d = _fakeSales30d();          // ventes des 30 derniers jours (totaux)
    final movements = _fakeMovements();        // derniers mouvements
    final supplier = _fakeSupplier();          // infos fournisseur & réappro
    final lowThreshold = 20;                   // seuil bas fictif
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          // ===== Derniers mouvements  (AVEC CHEVRON -> PAGE DÉDIÉE)
          Row(
            children: [
                  ...
                  Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (_) => MovementListScreen(
                        ...
                        sku: article.sku,
                        allMovements: _toMovementItems(_fakeMovements()),
                      ),
                    ),
                  );
                },
  ...
  _SalesSummary _fakeSales30d() {
    // ventes fictives sur 30 jours
    final qty = 27;
    final revenue = qty * article.sellPrice;
    return _SalesSummary(totalQty: qty, revenue: revenue);
  }

  List<_Movement> _fakeMovements() => [
        _Movement(type: _MoveType.out, qty: 3, date: DateTime.now().subtract(const Duration(days: 1)), reason: 'Vente POS #1052'),
        _Movement(type: _MoveType.inn, qty: 10, date: DateTime.now().subtract(const Duration(days: 4)), reason: 'Réception PO-784'),
        _Movement(type: _MoveType.adjust, qty: -1, date: DateTime.now().subtract(const Duration(days: 6)), reason: 'Inventaire'),
        _Movement(type: _MoveType.out, qty: 2, date: DateTime.now().subtract(const Duration(days: 8)), reason: 'Vente POS #1038'),
      ];
  ...
  _SupplierInfo _fakeSupplier() => _SupplierInfo(
        ...
        leadDays: 5,
        moq: 10,
        weightedCost: article.buyPrice * 0.98, // fictif
        hasOpenPo: true,
        openPoQty: 15,
        eta: 'J+4',
      );
  ...
  /// Conversion vers les éléments attendus par la page dédiée (type public MovementItem).
  List<MovementItem> _toMovementItems(List<_Movement> ms) {
    return ms
        .map(
          (m) => MovementItem(
            type: switch (m.type) {
...
class _Movement {
  ...
  final DateTime date;
  final String reason;
  const _Movement({required this.type, required this.qty, required this.date, required this.reason});
}
...
// ===== Petits badges / utilitaires manquants =================================

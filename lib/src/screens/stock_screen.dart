...
class _StockScreenState extends State<StockScreen>
  ...
  // Données fictives
  // -----------------------
  final List<_Article> _all = const [
    // Téléphones
    _Article(cat: _Cat.phones, name: 'iPhone 14 128 Go', sku: 'IP14-128-BLK', buyPrice: 650, sellPrice: 899, qty: 12),
    _Article(cat: _Cat.phones, name: 'Samsung Galaxy S23', sku: 'SGS23-128', buyPrice: 540, sellPrice: 799, qty: 9),
    _Article(cat: _Cat.phones, name: 'Xiaomi Redmi Note 13', sku: 'RN13-256', buyPrice: 190, sellPrice: 279, qty: 24),
    _Article(cat: _Cat.phones, name: 'Tecno Spark 20', sku: 'TSP20-64', buyPrice: 95, sellPrice: 149, qty: 30),

    // Accessoires
    _Article(cat: _Cat.accessories, name: 'Coque Silicone (iPhone 14)', sku: 'CASE-IP14-SIL', buyPrice: 4.2, sellPrice: 12.9, qty: 140),
    _Article(cat: _Cat.accessories, name: 'Verre trempé (universel)', sku: 'GLASS-UNI', buyPrice: 1.2, sellPrice: 5.0, qty: 220),
    _Article(cat: _Cat.accessories, name: 'Chargeur 20W USB-C', sku: 'CHG-20W', buyPrice: 6.8, sellPrice: 19.9, qty: 85),
    _Article(cat: _Cat.accessories, name: 'Câble USB-C 1m', sku: 'CB-USB-C-1M', buyPrice: 1.9, sellPrice: 7.9, qty: 160),

    // Tablettes
    _Article(cat: _Cat.tablets, name: 'iPad 10e Gen 64 Go', sku: 'IPAD10-64', buyPrice: 340, sellPrice: 499, qty: 7),
    _Article(cat: _Cat.tablets, name: 'Samsung Tab A9', sku: 'STABA9-64', buyPrice: 140, sellPrice: 219, qty: 10),

    // Wearables
    _Article(cat: _Cat.wearables, name: 'Apple Watch SE', sku: 'AW-SE-40', buyPrice: 190, sellPrice: 299, qty: 6),
    _Article(cat: _Cat.wearables, name: 'Xiaomi Band 8', sku: 'XB8', buyPrice: 21, sellPrice: 39.9, qty: 35),
  ];

  _Cat? _filter; // null = Tous
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Stats par catégorie (chips cliquables)
                ...
                return Ink(
                  ...
                  child: ListTile(
                    ...
                    subtitle: Padding(
                      ...
                      child: Text(
                        '${_catName(it.cat)} • SKU: ${it.sku} • Achat: ${_money(it.buyPrice)}',
  ...
  // Ouvre la feuille de stats globales.
  void _openStatsSheet(_Stats statsAll) {
    final byCat = {
      for (final c in _Cat.values) c: _statsFor(_byCat(c)),
    };
  ...
  // -----------------------
  // Transition personnalisée : glissement droite → gauche
  // -----------------------
    ...
    return PageRouteBuilder(
      ...
      transitionsBuilder: (context, animation, secondary, child) {
        final tween = Tween<Offset>(
          begin: const Offset(1.0, 0.0), // commence hors écran à droite
          end: Offset.zero,
  ...
  // Stats helpers
  // -----------------------
  _Stats _statsFor(List<_Article> list) {
    double cost = 0;
    double retail = 0;
    int qty = 0;
    int low = 0;
  ...
  List<_Article> _filtered() {
    if (_filter == null) return _all;
    return _all.where((a) => a.cat == _filter).toList();
  }

  List<_Article> _byCat(_Cat c) => _all.where((a) => a.cat == c).toList();
...
// Modèles & Widgets
// -----------------------
enum _Cat { phones, accessories, tablets, wearables }
...
class _Stats {
  final double valueCost;   // Valeur d’inventaire (coût)
  final double valueRetail; // Valeur potentielle de vente
  final int totalQty;
  final int lowCount;
...
class _CategoryChips extends StatelessWidget {
  ...
  final String Function(_Cat) builderLabel;
  final Widget Function(_Cat) builderTrailing;
